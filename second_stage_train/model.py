...
class Seq2Seq(nn.Module):
    ...
    def __init__(self, encoder, decoder, config, beam_size=None, max_length=None, sos_id=None, eos_id=None, l2_norm=False, fusion=True):
    ...
    def forward(self, source_ids=None, source_mask=None, similarity_ids=None, similarity_mask=None, target_ids=None, target_mask=None):
        ...
        similarity_outputs = self.encoder(similarity_ids, attention_mask=similarity_mask)
...
class BASHEXPLAINER():
    def __init__(self, codebert_path, decoder_layers, fix_encoder, beam_size, max_source_length, max_target_length, load_model_path, l2_norm, fusion):
        ...
        self.model = Seq2Seq(encoder=encoder, decoder=decoder, config=config,
                        ...
                        sos_id=self.tokenizer.cls_token_id, eos_id=self.tokenizer.sep_token_id, l2_norm=l2_norm, fusion=fusion)
    ...
    def train(self, train_filename, train_batch_size, num_train_epochs, learning_rate,
        ...
        train_data = TensorDataset(all_source_ids, all_source_mask, all_similarity_ids, all_similarity_mask, all_target_ids, all_target_mask)
        ...
        for epoch in range(num_train_epochs):
            ...
            for batch in bar:
                ...
                loss, _, _ = self.model(source_ids=source_ids, source_mask=source_mask, similarity_ids=similarity_ids, similarity_mask=similarity_mask,
            ...
            if do_eval==True:
                ...
                if 'dev_loss' in dev_dataset:
                    ...
                    eval_data = TensorDataset(all_source_ids, all_source_mask, all_similarity_ids, all_similarity_mask, all_target_ids, all_target_mask)
                ...
                for batch in tqdm(eval_dataloader, total=len(eval_dataloader)):
                    ...
                    with torch.no_grad():
                        _, loss, num = self.model(source_ids=source_ids, source_mask=source_mask, similarity_ids = similarity_ids,
                                                  similarity_mask = similarity_mask, target_ids=target_ids, target_mask=target_mask)
                ...
                for batch in tqdm(eval_dataloader, total=len(eval_dataloader)):
                    ...
                    with torch.no_grad():
                        preds = self.model(source_ids=source_ids, source_mask=source_mask, similarity_ids=similarity_ids, similarity_mask=similarity_mask)
                        for pred in preds:
                            t = pred[0].cpu().numpy()
                            t = list(t)
                            if 0 in t:
    ...
    def test(self, test_filename, test_batch_size, output_dir):
        ...
        for idx, file in enumerate(files):
            ...
            df = pd.DataFrame(csv_true_list)
            df.to_csv(os.path.join(output_dir, "BASHEXPLAINER_hyp.csv"), index=False, header=None)

            df = pd.DataFrame(csv_pred_list)
            df.to_csv(os.path.join(output_dir, "BASHEXPLAINER_ref.csv"), index=False, header=None)

            metrics_dict = compute_metrics(hypothesis=os.path.join(output_dir, "BASHEXPLAINER_hyp.csv"),
                                           references=[os.path.join(output_dir, "BASHEXPLAINER_ref.csv")], no_skipthoughts=True, no_glove=True)
