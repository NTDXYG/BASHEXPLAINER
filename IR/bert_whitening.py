...
MODEL_NAME = "D:\\chinese-roberta\\codebert-base"
...
def sents_to_vecs(sents, tokenizer, model, ir=False):
    vecs = []
    with torch.no_grad():
        if(ir==True):
            for sent in sents:
...
def main():
    ...
    df = pd.read_csv("../data/first_stage_data/train.csv")
    ...
    import pickle
    f = open('./model/bash_code_vector_whitening.pkl', 'wb')
    pickle.dump(vecs_func_body, f)
    f.close()
    f = open('./model/bash_kernel.pkl', 'wb')
    pickle.dump(kernel, f)
    f.close()
    f = open('./model/bash_bias.pkl', 'wb')
    pickle.dump(bias, f)
    f.close()
